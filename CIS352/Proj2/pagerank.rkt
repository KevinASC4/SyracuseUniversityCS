...
(define (mk-initial-pagerank graph)
  (define list (make-list graph))
  (define (create-hash list)
    (define hashValue (/ 1 (length (make-list graph))))
    (match list
      ['() '()]
      [`(,hd . ,rest) (cons (cons hd hashValue) (create-hash rest))])) 
  (make-immutable-hash (create-hash (make-list graph))))
...
(define (step-pagerank pr d graph)
  (let ([newGraph (sort (make-list graph) symbol<?)]
    ...
    (hash-remove (updateHash pr d graph newGraph initialHash) 123456789)))
...
;; Computes the entire equation
  (define (computeEquation pr d graph node)
    (+ (computeDF d graph) (* d (computePRSum pr (set->list (get-backlinks graph node)) graph))))
